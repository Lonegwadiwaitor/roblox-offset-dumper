...
struct Compilerator
{
    ...
    Compilerator(BytecodeBuilder& bytecode, const CompileOptions& options)
        : bytecode(bytecode)
        , options(options)
        , functions(nullptr)
        , locals(nullptr)
    ...
    uint8_t getUpval(AstLocal* local)
    {
        ...
        if (upvals.size() >= kMaxUpvalueCount)
            CompileError::raise(
                local->location, xor("Out of upvalue registers when trying to allocate %s: exceeded limit %d"), local->name.value, kMaxUpvalueCount);
    ...
    {
        LUAU_TIMETRACE_SCOPE(xor("Compilerator::compileFunction"), xor("Compilerator"));

        if (func->debugname.value)
            LUAU_TIMETRACE_ARGUMENT(xor("name"), func->debugname.value);
    ...
    void compileExprCall(AstExprCall* expr, uint8_t target, uint8_t targetCount, bool targetTop = false, bool multRet = false)
    {
        ...
        if (expr->self)
        {
            ...
            if (cid < 0)
                CompileError::raise(fi->location, xor("Exceeded constant limit; simplify the code to compile"));
        ...
        else if (bfid >= 0)
        {
            ...
            if (!bytecode.patchSkipC(fastcallLabel, callLabel))
                CompileError::raise(expr->func->location, xor("Exceeded jump distance limit; simplify the code to compile"));
        }
    ...
    void compileExprFunction(AstExprFunction* expr, uint8_t target)
    {
        ...
        if (pid < 0)
            CompileError::raise(expr->location, xor("Exceeded closure limit; simplify the code to compile"));
    ...
    LuauOpcode getUnaryOp(AstExprUnary::Op op)
    {
        ...
        default:
            LUAU_ASSERT(!xor("Unexpected unary operation"));
            return LOP_NOP;
        }
    }
    ...
    LuauOpcode getBinaryOpArith(AstExprBinary::Op op, bool k = false)
    {
        ...
        default:
            LUAU_ASSERT(!xor("Unexpected binary operation"));
            return LOP_NOP;
        }
    }
    ...
    int32_t getConstantNumber(AstExpr* node)
    {
        ...
        if (c && c->type == Constant::Type_Number)
        {
            ...
            if (cid < 0)
                CompileError::raise(node->location, xor("Exceeded constant limit; simplify the code to compile"));
    ...
    int32_t getConstantIndex(AstExpr* node)
    {
        ...
        default:
            LUAU_ASSERT(!xor("Unexpected constant type"));
            return -1;
        }
    ...
    void compileExprTable(AstExprTable* expr, uint8_t target, bool targetTemp)
    {
        ...
        if (arraySize == 0 && indexSize == 0 && hashSize == recordSize && recordSize >= 1 && recordSize <= BytecodeBuilder::TableShape::kMaxLength)
        {
            ...
            for (size_t i = 0; i < expr->items.size; ++i)
            {
                ...
                if (cid < 0)
                    CompileError::raise(ckey->location, xor("Exceeded constant limit; simplify the code to compile"));
            ...
            if (tid < 0)
                CompileError::raise(expr->location, xor("Exceeded constant limit; simplify the code to compile"));
    ...
    void compileExpr(AstExpr* node, uint8_t target, bool targetTemp = false)
    {
        ...
        {
            LUAU_ASSERT(!xor("Unknown expression type"));
        }
    }
    ...
    LValue compileLValue(AstExpr* node, RegScope& rs)
    {
        ...
        {
            LUAU_ASSERT(!xor("Unknown assignment expression"));
    ...
    void compileLValueUse(const LValue& lv, uint8_t reg, bool set)
    {
        ...
        case LValue::Kind_Global:
        {
            ...
            if (cid < 0)
                CompileError::raise(lv.location, xor("Exceeded constant limit; simplify the code to compile"));
        ...
        default:
            LUAU_ASSERT(!xor("Unknown lvalue kind"));
        }
    }
    ...
    void compileStatForIn(AstStatForIn* stat)
    {
        ...
        if (options.optimizationLevel >= 1 && stat->vars.size <= 2)
        {
            if (stat->values.size == 1 && stat->values.data[0]->is<AstExprCall>())
            {
                ...
                if (builtin.isGlobal(xor("ipairs"))) // for .. in ipairs(t)
                {
                    skipOp = LOP_FORGPREP_INEXT;
                    loopOp = LOP_FORGLOOP_INEXT;
                }
                else if (builtin.isGlobal(xor("pairs"))) // for .. in pairs(t)
                {
            ...
            else if (stat->values.size == 2)
            {
                ...
                if (builtin.isGlobal(xor("next"))) // for .. in next,t
                {
                    skipOp = LOP_FORGPREP_NEXT;
                    loopOp = LOP_FORGLOOP_NEXT;
                }
    ...
    void compileStatCompoundAssign(AstStatCompoundAssign* stat)
    {
        ...
        default:
            LUAU_ASSERT(!xor("Unexpected compound assignment operation"));
        }
    ...
    void compileStat(AstStat* node)
    {
        ...
        {
            LUAU_ASSERT(!xor("Unknown statement type"));
        }
    }
    ...
    void validateContinueUntil(AstStat* cont, AstExpr* condition)
    {
        ...
        if (visitor.undef)
            CompileError::raise(condition->location,
                xor("Local %s used in the repeat..until condition is undefined because continue statement on line %d jumps over it"),
    ...
    void pushLocal(AstLocal* local, uint8_t reg)
    {
        if (localStack.size() >= kMaxLocalCount)
            CompileError::raise(
                local->location, xor("Out of local registers when trying to allocate %s: exceeded limit %d"), local->name.value, kMaxLocalCount);
    ...
    void patchJump(AstNode* node, size_t label, size_t target)
    {
        if (!bytecode.patchJumpD(label, target))
            CompileError::raise(node->location, xor("Exceeded jump distance limit; simplify the code to compile"));
    }
    ...
    void patchLoopJumps(AstNode* node, size_t oldJumps, size_t endLabel, size_t contLabel)
    {
        ...
        for (size_t i = oldJumps; i < loopJumps.size(); ++i)
        {
            ...
            default:
                LUAU_ASSERT(!xor("Unknown loop jump type"));
            }
        }
    }
    ...
    uint8_t allocReg(AstNode* node, unsigned int count)
    {
        ...
        if (top + count > kMaxRegisterCount)
            CompileError::raise(node->location, xor("Out of registers when trying to allocate %d registers: exceeded limit %d"), count, kMaxRegisterCount);
    ...
    struct FenvVisitor : AstVisitor
    {
        ...
        {
            if (node->name == xor("getfenv"))
                getfenvUsed = true;
            if (node->name == xor("setfenv"))
                setfenvUsed = true;
    ...
    struct FunctionVisitor : AstVisitor
    {
        ...
        FunctionVisitor(Compilerator* self, std::vector<AstExprFunction*>& functions)
            : self(self)
            , functions(functions)
        {
        }
    ...
    {
        UndefinedLocalVisitor(Compilerator* self)
            : self(self)
            , undef(nullptr)
        {
        }
    ...
    {
        ConstUpvalueVisitor(Compilerator* self)
            : self(self)
        {
        }
    ...
    struct RegScope
    {
        ...
        // discarded
        RegScope(Compilerator* self, unsigned int top)
            : self(self)
            , oldTop(self->regTop)
        {
...
{
    LUAU_TIMETRACE_SCOPE(xor("compileOrThrow"), xor("Compilerator"));

    Compilerator compiler(bytecode, options);
    ...
    if (options.optimizationLevel >= 1 && (names.get(xor("getfenv")).value || names.get(xor("setfenv")).value))
    {
        Compilerator::FenvVisitor fenvVisitor(compiler.getfenvUsed, compiler.setfenvUsed);
    ...
    std::vector<AstExprFunction*> functions;
    Compilerator::FunctionVisitor functionVisitor(&compiler, functions);
    ...
    AstExprFunction main(root->location, /*generics= */ AstArray<AstGenericType>(), /*genericPacks= */ AstArray<AstGenericTypePack>(),
        /* self= */ nullptr, AstArray<AstLocal*>(), /* vararg= */ Location(), root, /* functionDepth= */ 0, /* debugname= */ AstName());
...
std::string compile(const std::string& source, bool* isError, const CompileOptions& options, const ParseOptions& parseOptions, BytecodeEncoder* encoder)
{
    LUAU_TIMETRACE_SCOPE(xor("compile"), xor("Compilerator"));
    ...
    if (!result.errors.empty())
    {
        ...
        const ParseError& parseError = result.errors.front();
        std::string error = format(xor(":%d: %s"), parseError.getLocation().begin.line + 1, parseError.what());
        *isError = true;
        return error;
    }
    ...
    {
        std::string error = format(xor(":%d: %s"), e.getLocation().begin.line + 1, e.what());
        *isError = true;
        return error;
    }
}
