...
static std::string kReserved[] = {xor("and"), xor("break"), xor("do"), xor("else"), xor("elseif"), xor("end"), xor("false"), xor("for"), xor("function"), xor("if"), xor("in"), xor("local"), xor("nil"), xor("not"), xor("or"),
    xor("repeat"), xor("return"), xor("then"), xor("true"), xor("until"), xor("while")};
...
std::string Lexeme::toString() const
{
    ...
    case RawString:
    case QuotedString:
        return data ? format(xor("\"%.*s\""), length, data) : xor("string");

    case Number:
        return data ? format(xor("'%.*s'"), length, data) : xor("number");

    case Name:
        return name ? format(xor("'%s'"), name) : xor("identifier");

    case Comment:
        return xor("comment");

    case BrokenString:
        return xor("malformed string");

    case BrokenComment:
        return xor("unfinished comment");

    case BrokenUnicode:
        if (codepoint)
        {
            if (const char* confusable = findConfusable(codepoint))
                return format(xor("Unicode character U+%x (did you mean '%s'?)"), codepoint, confusable);

            return format(xor("Unicode character U+%x"), codepoint);
        }
        else
        {
            return xor("invalid UTF-8 sequence");
        }

    default:
        if (type < Char_END)
            return format(xor("'%c'"), type);
        else if (type >= Reserved_BEGIN && type < Reserved_END)
            return format(xor("'%s'"), kReserved[type - Reserved_BEGIN]);
        else
            return xor("<unknown>");
    }
...
AstNameTable::AstNameTable(Allocator& allocator)
    ...
    , allocator(allocator)
{
    static_assert(std::size(kReserved) == Lexeme::Reserved_END - Lexeme::Reserved_BEGIN);

    for (int i = Lexeme::Reserved_BEGIN; i < Lexeme::Reserved_END; ++i)
        addStatic(kReserved[i - Lexeme::Reserved_BEGIN].c_str(), static_cast<Lexeme::Type>(i));
}

AstName AstNameTable::addStatic(const char* name, Lexeme::Type type)
{
	Entry entry = {AstName(name), uint32_t(strlen(name)), type};
...
{
	Entry key = {AstName(name), uint32_t(length), Lexeme::Eof};
