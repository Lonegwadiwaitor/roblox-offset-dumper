...
std::pair<uint16_t, Compiler::TimeTrace::ThreadContext&> createScopeData(const char* name, const char* category)
{
    uint16_t token = createToken(Compiler::TimeTrace::getGlobalContext(), name, category);
    return {token, Compiler::TimeTrace::getThreadContext()};
}
} // namespace TimeTrace
} // namespace Compiler
