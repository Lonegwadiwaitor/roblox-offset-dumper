    ...
    , localMap(AstName())
{
    ...
    nameSelf = names.addStatic(xor("self"));
    nameNumber = names.addStatic(xor("number"));
    nameError = names.addStatic(errorName);
    nameNil = names.getOrAdd(xor("nil")); // nil is a reserved keyword
...
AstStat* Parser::parseStat()
{
    ...
    if (options.allowTypeAnnotations)
    {
        ...
        if (ident == xor("export") && lexer.current().type == Lexeme::Name && AstName(lexer.current().name) == xor("type"))
        {
            nextLexeme();
    ...
    auto err = reportStatError(expr->location, copy({ expr }), {}, xor("Incomplete statement: expected assignment or a function call"));
...
AstStat* Parser::parseIf()
{
    ...
    bool hasThen = expectAndConsume(Lexeme::ReservedThen, xor("if statement"));
    ...
    {
        unsigned int recursionCounterOld = recursionCounter;
        incrementRecursionCounter(xor("elseif"));
...
AstStat* Parser::parseWhile()
{
    ...
    bool hasDo = expectAndConsume(Lexeme::ReservedDo, xor("while loop"));
...
AstStat* Parser::parseBreak()
{
    ...
    if (functionStack.back().loopDepth == 0)
        return reportStatError(start, {}, copy<AstStat*>({allocator.alloc<AstStatBreak>(start)}), xor("break statement must be inside a loop"));
...
AstStat* Parser::parseContinue(const Location& start)
{
    ...
    if (functionStack.back().loopDepth == 0)
        return reportStatError(start, {}, copy<AstStat*>({allocator.alloc<AstStatContinue>(start)}), xor("continue statement must be inside a loop"));
...
AstStat* Parser::parseFor()
{
    ...
    if (lexer.current().type == '=')
    {
        ...
        expectAndConsume(',', xor("index range"));
        ...
        Lexeme matchDo = lexer.current();
        bool hasDo = expectAndConsume(Lexeme::ReservedDo, xor("for loop"));
    ...
    else
    {
        ...
        bool hasIn = expectAndConsume(Lexeme::ReservedIn, xor("for loop"));
...
AstStat* Parser::parseFunctionStat()
{
    ...
    AstExpr* expr = parseNameExpr(xor("function name"));
    ...
    while (lexer.current().type == '.')
    {
        ...
        Name name = parseName(xor("field name"));
        ...
        incrementRecursionCounter(xor("function name"));
    }
    ...
    if (lexer.current().type == ':')
    {
        ...
        Name name = parseName(xor("method name"));
...
AstStat* Parser::parseLocal()
{
    ...
    if (lexer.current().type == Lexeme::ReservedFunction)
    {
        ...
        Name name = parseName(xor("variable name"));
...
AstStat* Parser::parseTypeAlias(const Location& start, bool exported)
{
    ...
    auto name = parseNameOpt(xor("type name"));
    ...
    expectAndConsume('=', xor("type alias"));
...
AstDeclaredClassProp Parser::parseDeclaredClassMethod()
{
    ...
    nextLexeme();
    Location start = lexer.current().location;
    Name fnName = parseName(xor("function name"));
    ...
    expectAndConsume('(', xor("function parameter list start"));
    ...
    if (args.size() == 0 || args[0].name.name != "self" || args[0].annotation != nullptr)
    {
        return AstDeclaredClassProp{fnName.name,
            reportTypeAnnotationError(Location(start, end), {}, /*isMissing*/ false, xor("'self' must be present as the unannotated first parameter")),
            true};
    }
    ...
    for (size_t i = 1; i < args.size(); ++i)
    {
        ...
        if (args[i].annotation)
            ...
            vars.push_back(reportTypeAnnotationError(
                Location(start, end), {}, /*isMissing*/ false, xor("All declaration parameters aside from 'self' must be annotated")));
    }

    if (vararg && !varargAnnotation)
        report(start, xor("All declaration parameters aside from 'self' must be annotated"));
...
AstStat* Parser::parseDeclaration(const Location& start)
{
    ...
    {
        nextLexeme();
        Name globalName = parseName(xor("global function name"));

        auto [generics, genericPacks] = parseGenericTypeList(/* withDefaultValues= */ false);

        Lexeme matchParen = lexer.current();

        expectAndConsume('(', xor("global function declaration"));
        ...
        {
            if (!args[i].annotation)
                return reportStatError(Location(start, end), {}, {}, xor("All declaration parameters must be annotated"));
    ...
    else if (AstName(lexer.current().name) == xor("class"))
    {
        nextLexeme();
        Location classStart = lexer.current().location;
        Name className = parseName(xor("class name"));
        std::optional<AstName> superName = std::nullopt;

        if (AstName(lexer.current().name) == xor("extends"))
        {
            nextLexeme();
            superName = parseName(xor("superclass name")).name;
        }
        ...
        while (lexer.current().type != Lexeme::ReservedEnd)
        {
            ...
            {
                Name propName = parseName(xor("property name"));
                expectAndConsume(':', xor("property type annotation"));
    ...
    else if (auto globalName = parseNameOpt(xor("global variable name")))
    {
        expectAndConsume(':', xor("global variable declaration"));
    ...
    {
        return reportStatError(start, {}, {}, xor("declare must be followed by an identifier, 'function', or 'class'"));
    }
...
AstStat* Parser::parseAssignment(AstExpr* initial)
{
    ...
    if (!isExprLValue(initial))
        initial = reportExprError(initial->location, copy({initial}), xor("Assigned expression must be a variable or a field"));
    ...
    while (lexer.current().type == ',')
    {
        ...
        if (!isExprLValue(expr))
            expr = reportExprError(expr->location, copy({expr}), xor("Assigned expression must be a variable or a field"));

        vars.push_back(expr);
    }

    expectAndConsume('=', xor("assignment"));
...
Parser::Binding Parser::parseBinding()
{
    ...
    auto name = parseNameOpt(xor("variable name"));
...
std::optional<AstTypeList> Parser::parseOptionalReturnTypeAnnotation()
{
    ...
    if (options.allowTypeAnnotations && lexer.current().type == ':')
    {
        ...
        if (lexer.current().type == ',')
        {
            report(lexer.current().location, xor("Expected a statement, got ','; did you forget to wrap the list of return types in parentheses?"));
...
std::pair<Location, AstTypeList> Parser::parseReturnTypeAnnotation()
{
    ...
    incrementRecursionCounter(xor("type annotation"));
...
AstTableIndexer* Parser::parseTableIndexerAnnotation()
{
    ...
    expectAndConsume(':', xor("table field"));
...
AstType* Parser::parseTableTypeAnnotation()
{
    ...
    expectAndConsume('{', xor("table type"));
    ...
    while (lexer.current().type != '}')
    {
            ...
            (lexer.lookahead().type == Lexeme::RawString || lexer.lookahead().type == Lexeme::QuotedString))
        {
            ...
            if (chars && !containsNull)
                ...
                report(begin.location, xor("String literal contains malformed escape sequence"));
        }
        else if (lexer.current().type == '[')
        {
            if (indexer)
            {
                ...
                report(badIndexer->location, xor("Cannot have more than one table indexer"));
            }
            else
            {
        ...
        {
            auto name = parseNameOpt(xor("table field"));
...
AstTypeOrPack Parser::parseFunctionTypeAnnotation(bool allowPack)
{
    ...
    expectAndConsume('(', xor("function parameters"));
...

{
    ...
    {
        report(Location(begin.location, lexer.previousLocation()), xor("Expected '->' after '()' when parsing function type; did you mean 'nil'?"));

        return allocator.alloc<AstTypeReference>(begin.location, std::nullopt, nameNil);
    }
    else
    {
        expectAndConsume(Lexeme::SkinnyArrow, xor("function type"));
    }
...
AstType* Parser::parseTypeAnnotation(TempVector<AstType*>& parts, const Location& begin)
{
    ...
    if (isUnion && isIntersection)
    {
        return reportTypeAnnotationError(Location(begin, parts.back()->location), copy(parts), /*isMissing*/ false,
            xor("Mixing union and intersection types is not allowed; consider wrapping in parentheses."));
    }
    ...
    LUAU_ASSERT(false);
    ParseError::raise(begin, xor("Composite type was not an intersection or union."));
...
AstTypeOrPack Parser::parseSimpleTypeAnnotation(bool allowPack)
{
    ...
    else if (FFlag::LuauParseSingletonTypes && (lexer.current().type == Lexeme::RawString || lexer.current().type == Lexeme::QuotedString))
    {
        if (std::optional<AstArray<char>> value = parseCharArray())
        {
            ...
            return {reportTypeAnnotationError(begin, {}, /*isMissing*/ false, xor("String literal contains malformed escape sequence"))};
    }
    else if (FFlag::LuauParseSingletonTypes && lexer.current().type == Lexeme::BrokenString)
    {
        Location location = lexer.current().location;
        nextLexeme();
        return {reportTypeAnnotationError(location, {}, /*isMissing*/ false, xor("Malformed string"))};
    }
    else if (lexer.current().type == Lexeme::Name)
    {
        std::optional<AstName> prefix;
        Name name = parseName(xor("type name"));
        ...
        if (lexer.current().type == '.')
        {
            ...
            prefix = name.name;
            name = parseIndexName(xor("field name"), pointPosition);
        }
        else if (name.name == xor("typeof"))
        {
            Lexeme typeofBegin = lexer.current();
            expectAndConsume('(', xor("typeof type"));
    ...
    else
    {
        ...
        return {reportTypeAnnotationError(location, {}, /*isMissing*/ true, xor("Expected type, got %s"), lexer.current().toString().c_str()), {}};
    }
...
AstTypePack* Parser::parseVariadicArgumentAnnotation()
{
    ...
    {
        Name name = parseName(xor("generic name"));
        Location end = lexer.current().location;

        // This will not fail because of the lookahead guard.
        expectAndConsume(Lexeme::Dot3, xor("generic type pack annotation"));
...
std::optional<AstExprUnary::Op> Parser::checkUnaryConfusables()
{
    ...
    if (curr.type == '!')
    {
        report(start, xor("Unexpected '!', did you mean 'not'?"));
        return AstExprUnary::Not;
    }
...
std::optional<AstExprBinary::Op> Parser::checkBinaryConfusables(const BinaryOpPriority binaryPriority[], unsigned int limit)
{
    ...
    {
        nextLexeme();
        report(Location(start, next.location), xor("Unexpected '&&', did you mean 'and'?"));
        return AstExprBinary::And;
    }
    else if (curr.type == '|' && next.type == '|' && curr.location.end == next.location.begin && binaryPriority[AstExprBinary::Or].left > limit)
    {
        nextLexeme();
        report(Location(start, next.location), xor("Unexpected '||', did you mean 'or'?"));
        return AstExprBinary::Or;
    }
    ...
    {
        nextLexeme();
        report(Location(start, next.location), xor("Unexpected '!=', did you mean '~='?"));
        return AstExprBinary::CompareNe;
    }
...
AstExpr* Parser::parseExpr(unsigned int limit)
{
    ...
    incrementRecursionCounter(xor("expression"));
...
AstExpr* Parser::parsePrimaryExpr(bool asStatement)
{
    ...
    while (true)
    {
        ...
        else if (lexer.current().type == ':')
        {
            ...
            Name index = parseIndexName(xor("method name"), opPosition);
        ...
        else if (lexer.current().type == '(')
        {
            ...
            if (!asStatement && expr->location.end.line != lexer.current().location.begin.line)
            {
                report(lexer.current().location,
                    xor("Ambiguous syntax: this looks like an argument list for a function call, but could also be a start of "
...
AstExpr* Parser::parseFunctionArgs(AstExpr* func, bool self, const Location& selfLocation)
{
    ...
    if (lexer.current().type == '(')
    {
        ...
        {
            report(lexer.current().location, xor("Ambiguous syntax: this looks like an argument list for a function call, but could also be a start of "
                                             "new statement; use ';' to separate statements"));
        }
    ...
    else
    {
        ...
        {
            return reportExprError(func->location, copy({func}), xor("Expected function call arguments after '('"));
        }
        else
        {
            return reportExprError(Location(func->location.begin, lexer.current().location.begin), copy({func}),
                xor("Expected '(', '{' or <string> when parsing function call, got %s"), lexer.current().toString().c_str());
        }
    }
        ...
        while (true)
        {
            ...
            if (lexer.current().type == Lexeme::Dot3 || (FFlag::LuauParseRecoverTypePackEllipsis && seenPack))
            {
                ...
                if (FFlag::LuauParseRecoverTypePackEllipsis && lexer.current().type != Lexeme::Dot3)
                    report(lexer.current().location, xor("Generic types come before generic type packs"));
                else
                    nextLexeme();
                ...
                if (withDefaultValues && lexer.current().type == '=')
                {
                    ...
                    else if (lexer.current().type == '(')
                    {
                        ...
                        if (type)
                            report(Location(packBegin.location.begin, lexer.previousLocation().end), xor("Expected type pack after '=', got type"));
                ...
                {
                    if (seenDefault)
                        report(lexer.current().location, xor("Expected default type pack after type pack name"));
            ...
            else
            {
                ...
                {
                    if (seenDefault)
                        report(lexer.current().location, xor("Expected default type after type name"));
...
LUAU_NOINLINE void Parser::expectAndConsumeFail(Lexeme::Type type, const char* context)
{
    ...
    if (context)
        report(lexer.current().location, xor("Expected %s when parsing %s, got %s"), typeString.c_str(), context, currLexemeString.c_str());
    else
        report(lexer.current().location, xor("Expected %s, got %s"), typeString.c_str(), currLexemeString.c_str());
...
LUAU_NOINLINE void Parser::expectMatchAndConsumeFail(Lexeme::Type type, const Lexeme& begin, const char* extra)
{
    ...
    if (lexer.current().location.begin.line == begin.location.begin.line)
        report(lexer.current().location, xor("Expected %s (to close %s at column %d), got %s%s"), typeString.c_str(), begin.toString().c_str(),
            begin.location.begin.column + 1, lexer.current().toString().c_str(), extra ? extra : "");
    else
        report(lexer.current().location, xor("Expected %s (to close %s at line %d), got %s%s"), typeString.c_str(), begin.toString().c_str(),
...
LUAU_NOINLINE void Parser::expectMatchEndAndConsumeFail(Lexeme::Type type, const Lexeme& begin)
{
    ...
    if (endMismatchSuspect.type != Lexeme::Eof && endMismatchSuspect.location.begin.line > begin.location.begin.line)
    {
        std::string suggestion =
            format(xor("; did you forget to close %s at line %d?"), endMismatchSuspect.toString().c_str(), endMismatchSuspect.location.begin.line + 1);
...
void Parser::incrementRecursionCounter(const char* context)
{
    ...
    {
        ParseError::raise(lexer.current().location, xor("Exceeded allowed recursion depth; simplify your %s to make the code compile"), context);
    }
...
void Parser::report(const Location& location, const char* format, va_list args)
{
    ...
    if (parseErrors.size() >= unsigned(FInt::LuauParseErrorLimit))
        ParseError::raise(location, xor("Reached error limit (%d)"), int(FInt::LuauParseErrorLimit));
}
...
LUAU_NOINLINE void Parser::reportNameError(const char* context)
{
    ...
    if (context)
        report(lexer.current().location, xor("Expected identifier when parsing %s, got %s"), context, lexer.current().toString().c_str());
    else
        report(lexer.current().location, xor("Expected identifier, got %s"), lexer.current().toString().c_str());
